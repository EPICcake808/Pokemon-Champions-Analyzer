...
- The standalone Vercel frontend cannot rely on the repo-root `CHANGELOG.md` being present at runtime, so keep the bundled fallback changelog in `web/src/lib/site-documents.ts` synced with the root changelog when shipping releases.

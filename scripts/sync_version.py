...
def replace_once(path: Path, pattern: str, replacement: str, optional: bool = False) -> bool:
...
def sync_all(version: str) -> list[str]:
    changed: list[str] = []
    # The pokemon-team-analyzer entry in uv.lock is an editable install
    # (source = { editable = "." }) and has no `version = ` line to pin, so a zero-match
    # result is expected and skipped. This still syncs if the lock ever becomes version-pinned.
    if replace_once(
        ROOT / "uv.lock",
